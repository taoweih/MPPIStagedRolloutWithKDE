...
from hydrax.simulation.deterministic import run_benchmark
...
def run_benchmark_memory_ant(
    ...
    """Run benchmark for MPPIMemoryContinuous with Ant-specific memory pretraining."""
    print(
    ...
    def get_true_cost(qpos):
        ...
        qpos_full = d.qpos.at[0:2].set(qpos)
        d = d.replace(qpos=qpos_full)
...
if __name__ == "__main__":
    ...
    def state_selection_function(state: mjx.Data) -> jax.Array:
        # Use torso xy position from generalized coordinates for density/memory state.
    ...
    for h in tqdm(range(Horizon_steps)):
        ...
        ctrl_list = [MPPI(task, num_samples=NUM_SAMPLES, noise_level=NOISE_LEVEL, temperature=TEMPERATURE,
                    ...
                    MPPIMemoryContinuous(task, num_samples=NUM_SAMPLES, noise_level=NOISE_LEVEL, temperature=TEMPERATURE,
                                    ...
                                    grid_min=-10.0, grid_max=10.0, online_learning_rate=1e-3,
        ...
        for j in range(len(ctrl_list)):
            ...
            if isinstance(ctrl, MPPIMemoryContinuous):
                num_success, control_freq, state_trajectory, control_trajectory, avg_success_iteration = run_benchmark_memory_ant(
                    ctrl,
                    mj_model,
                    mj_data,
                    frequency=50,
    ...
    save_dir = Path(ROOT)/"benchmark"/f"ant_benchmark_{special_name}_{curr_time}"
    ...
    # ==================== NUM_SAMPLES SWEEP ====================
    SWEEP_HORIZON = 0.2  # fixed horizon for the samples sweep
